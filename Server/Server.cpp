...
int main(int argc, char* argv[])
{
	...
	// stub 함수 정의, remote, rmiContext, key
	g_GameStub.Move_Function = [&srv] PARAM_TestGame_Move
	{
		// 게임 참여 확인
		if (map_HostID_player.find(remote) == map_HostID_player.end()) return true;
		...
		if (!player_moved) return true;

		// 이동 처리 및 아이템, 점수 갱신
		vector<Item> player_items = map_player_Items[player_no];
		PlayerInfo player_info = map_player_PlayerInfo[player_no];
		for (vector<Item>::iterator i = player_items.begin(); i != player_items.end(); ++i) {
			if (i->pos_x != player_info.pos_x || i->pos_y != player_info.pos_y) continue;
		...
		if (next_player >= player_no) next_player++;
		// 다음 플레이어 정보
		...
		do {
			next_coin_x = rand() % g_TileCol;
			next_coin_y = rand() % g_TileRow;
		} while (next_player_info.pos_x == next_coin_x && next_player_info.pos_y == next_coin_y);
		// 코인 생성
		next_player_items.push_back(Item(next_coin_x, next_coin_y));

		// 그룹 내 플레이어에게 업데이트 정보 전송
		g_GameProxy.SendPlayerInfo(g_groupHostID, RmiContext::FastEncryptedReliableSend, player_no, player_info);
		g_GameProxy.SendItems(g_groupHostID, RmiContext::FastEncryptedReliableSend, player_no, player_items);
		g_GameProxy.SendItems(g_groupHostID, RmiContext::FastEncryptedReliableSend, next_player, next_player_items);
	...
	srv->OnClientJoin = [&srv](CNetClientInfo *clientInfo){
		...
		for (int i = 0; i < MAX_PLAYERS; ++i) {
			if (set_player.find(i) != set_player.end()) continue;
			player_no = i;
		}
		if (player_no == -1) return;
		...
		// 플레이어 2명 이상 시 게임 시작
		if (set_player.size() >= 2) game_started = true;
	};

	// set a routine for client leave event.
	srv->OnClientLeave = [&srv](CNetClientInfo *clientInfo, ErrorInfo *errorInfo, const ByteArray& comment){
		HostID remote = clientInfo->m_HostID;
		...
		if (set_player.size() < 2) {
			...
			// 기존 플레이어 정보 초기화
			for (set<int>::const_iterator i = set_player.begin(); i != set_player.end(); ++i) {
		...
		// 모든 플레이어에게 정보 전송
		for (set<int>::const_iterator i = set_player.begin(); i != set_player.end(); ++i) {
			g_GameStub.SendItems(g_groupHostID, RmiContext::FastEncryptedReliableSend, *i, map_player_Items[*i]);
			g_GameStub.SendPlayerInfo(g_groupHostID, RmiContext::FastEncryptedReliableSend, *i, map_player_PlayerInfo[*i]);
		}
	};
